...
function gmdate( format, timestamp ) {
    ...
    var currentlyGmt = _isGmt2;
    _isGmt2 = true;
    var ret = phpdate(format, dt);
    _isGmt2 = currentlyGmt;
...
var formatters = {
    ...
    a: function(now){ return (_isGmt ? _gmDt.getUTCHours() : now.getHours()) < 12 ? "am" : "pm"; },
    A: function(now){ return (_isGmt ? _gmDt.getUTCHours() : now.getHours()) < 12 ? "AM" : "PM"; },
    B: function(now){ var tz = tzInfo(now, _isGmt2);
        // swatch time is GMT +01:00, ie timestamp 1 is 1am, '041'
        //var tm = now.getTime() - (_isGmt ? tz.offs : 0) * 60 * 1000 + 3600000;
        var tm = (_isGmt ? _gmDt.getTime() : now.getTime()) + 3600 * 1000;
        return pad3(Math.floor(1000 * (tm % 86400000) / 86400000)); },
    g: function(now){ return hour1to12(now); },
    // 
    G: function(now){ return (_isGmt ? _gmDt.getUTCHours() : now.getHours()); },
    H: function(now){ return pad2(_isGmt ? _gmDt.getUTCHours() : now.getHours()); },
    ...
    // 'e' returns UTC, 'T' returns GMT
    e: function(now){ var tz = tzInfo(now, _isGmt2); return (_isGmt ? 'UTC' : tz.tzname); },
    I: function(now){ var tz = tzInfo(now, _isGmt2); return (_isGmt ? 0 : tz.isDst ? 1 : 0); },
    O: function(now){ var tz = tzInfo(now, _isGmt2); return (_isGmt ? '+0000' : tz.sign + pad2(tz.h) + pad2(tz.m)); },
    P: function(now){ var tz = tzInfo(now, _isGmt2); return (_isGmt ? '+00:00' : tz.sign + pad2(tz.h) + ":" + pad2(tz.m)); },
    T: function(now){ var tz = tzInfo(now, _isGmt2); return (_isGmt ? 'GMT' : tz.tz); },
    Z: function(now){ var tz = tzInfo(now, _isGmt2); return (_isGmt ? 0 : -tz.offs * 60); },

    c: function(now){ var tz = tzInfo(now, _isGmt2); return phpdate("Y-m-d\\TH:i:sP", now); },
    r: function(now){ var tz = tzInfo(now, _isGmt2); return (_isGmt ? gmdate : phpdate)("D, d M Y H:i:s O", (_isGmt ? _gmDt : now)); },
    U: function(now){ var tz = tzInfo(now, _isGmt2); return Math.floor((_isGmt ? _gmDt.getTime() : now.getTime()) / 1000); },
};
...
function hour1to12( now ) {
    var hour = (_isGmt ? _gmDt.getUTCHours() : now.getHours()) % 12;
...
function tzInfo( now, isGmt ) {
    if (isGmt) {
        ...
        return {
            ...
            m: 0,
            isDst: 0,
            tz: 'GMT',          // php timezone abbrev is 'GMT'
            sign: '+',
            tzname: 'UTC',      // php timezone name is 'UTC'
        };
    }
