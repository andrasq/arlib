...
function gmdate( format, timestamp ) {
    ...
    var dt = _timestampToDt(timestamp);
    if (!currentlyGmt) {
        // since js always returns values formatted as if in localtime, adjust
        // dt so that localtime values from it will appear correct for GMT
        var tz = tzInfo(dt);
        // FIXME: if dt is at the end of DST, and GMT is already on DT,
        // then the offset calculated will be short by 1 hour!!
        // FIXME: this tries to correct for timezones west of GMT, need to check logic for east of
        // FIXME: work in progress, not done yet
        var offs1 = tz.offs;
        var dt2 = new Date(dt.getTime() + tz.offs * 60 * 1000);
        var offs2 = tzInfo(dt2).offs;
        var offs3 = tzInfo(new Date(dt2.getTime() + tz.offs * 60 * 1000)).offs;
        dt = new Date(dt.getTime() + offs2 * 60 * 1000);
    }
...
var formatters = {
    ...
    U: function(now){ var tz = tzInfo(now); return Math.floor((now.getTime() - (_isGmt ? tz.offs * 60 * 1000 : 0)) / 1000); },
};
