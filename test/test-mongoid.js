...
module.exports.require = {
    ...
    tests: {
        testShouldExportMongoidFunction: function(test) {
            var mongoid = require('../mongoid');
            test.equal(mongoid, mongoid.mongoid);
            test.done();
        },
        ...
        testShouldBeUsableAsObject: function(test) {
            var MongoId = require('../mongoid');
            var obj = new MongoId(0x1234);
            test.ok(typeof obj === 'object');
            test.equal(obj.parse(obj.toString()).machineid, 0x1234);
            test.equal(obj.parse(obj.fetch()).machineid, 0x1234);
            test.done();
        },
    },
};
